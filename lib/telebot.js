...
class TeleBot {
    ...
    constructor(cfg) {
        ...
        if (!cfg.token || cfg.token.split('416522274:AAFqTsF0YDQaGUIH2tL66RUWTjQOkqaxTwI').length != 2) {
...
bot.on('/idm', (msg) => {
  return bot.sendMessage(msg.from.id, `ÂíÏí ÚÏÏí ÔãÇ :    ${ msg.from.id }`);
});


bot.on('/test', (msg) => msg.reply.text('åäæÒ íÒí äãíÏæäã'));
bot.on('/hide', (msg) => msg.reply.text('ÈÑÇí äãÇíÔ ˜íÈæÑÏ ÇÒ /start ÇÓÊÝÇÏå ˜äíÏ', {replyMarkup: 'hide'}));
...
bot.on(['/start', '/back'], (msg) => {
    ...
    return bot.sendMessage(msg.from.id, '˜íÈæÑÏ äãÇíÇä ÔÏ', {replyMarkup});
...
bot.on('/hiden', msg => {
    return bot.sendMessage(
        msg.from.id, '˜íÈæÏ ãÎÝ ÔÏ! ÈÑÇí äãÇíÔ ˜íÈæÑÏ ÞÈáí ÇÒ /test æÈÑÇí ˜íÈæÑÏ ÇÕáí ÇÒ /start ÇÓÊÝÇÏå ˜äíÏ', {replyMarkup: 'hide'}
    );
});
